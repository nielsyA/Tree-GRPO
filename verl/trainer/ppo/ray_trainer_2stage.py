...
class RayPPOTrainer(object):
    ...
    def init_workers(self):
        ...
        elif self.config.algorithm.adv_estimator in ['grpo', 'tree', 'tree_inner', 'no_estimator', 'tree_per_token', 'tree_2norm']:
    ...
    def _load_checkpoint(self):
        if self.config.trainer.load_checkpoint == "disable":
            return 0

        checkpoint_folder = self.config.trainer.load_checkpoint_dir
        print(f"Load checkpoint from {checkpoint_folder}")

        self.actor_rollout_wg.load_checkpoint(checkpoint_folder)
    ...
    def fit(self):
        ...
        ts_m = self.config.actor_rollout_ref.rollout.ts_m
        ts_k = self.config.actor_rollout_ref.rollout.ts_k
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if self.global_steps == self.config.trainer.stage1_training_steps + 1:
                    print(f'[########]start stage 2 training at epoch {epoch}, step {self.global_steps}')
                    ...
                    gen_config = GenerationTreeSearchConfig(
                        ...
                        search_url = self.config.retriever.url,
                        topk = self.config.retriever.topk,
                        m = self.config.actor_rollout_ref.rollout.ts_m_s2,
                        n = self.config.actor_rollout_ref.rollout.ts_n_s2,
                        l = self.config.actor_rollout_ref.rollout.ts_l_s2,
                        k = self.config.actor_rollout_ref.rollout.ts_k_s2,
                        reward_mode = self.config.actor_rollout_ref.rollout.reward_mode,
                        expand_mode = self.config.actor_rollout_ref.rollout.expand_mode,
                    )
                    ...
                    ts_m = self.config.actor_rollout_ref.rollout.ts_m_s2
                    ts_k = self.config.actor_rollout_ref.rollout.ts_k_s2

                    adv_estimator = self.config.algorithm.adv_estimator_s2

                    # change batch size normalization
                    self.actor_rollout_wg.normalize_config_stage2()
                    self.ref_policy_wg.normalize_config_stage2()
